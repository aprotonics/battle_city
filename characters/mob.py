...
def create_path(start, goal):
    ...
    came_from, cost_so_far, iterations = a_star_search(Configuratuion.graph, start, goal)
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, enemy_id, x):
        ...
        self.mode1_duration = Configuratuion.enemy_mode1_duration
        self.rand_image = random.choice(Configuratuion.enemy_images)[0]
        ...
        self.speed = Configuratuion.enemy_speed
        self.speedx = 0
    ...
    def __init_mode2__(self):
        ...
        self.step = 0
        self.goal = (Configuratuion.player.graph_coordinate_x, Configuratuion.player.graph_coordinate_y)
        ...
        Configuratuion.enemies_mode1.remove(self)
        Configuratuion.enemies_mode2.add(self)
    ...
    def __init_mode3__(self):
        ...
        self.start = (self.graph_coordinate_x, self.graph_coordinate_y)
        self.step = 0
        self.goal1 = (Configuratuion.base.rect.x - 100, Configuratuion.base.rect.y)
        self.goal2 = (Configuratuion.base.rect.x, Configuratuion.base.rect.y - 100)
        self.goal3 = (Configuratuion.base.rect.x + 100, Configuratuion.base.rect.y)
        ...
        Configuratuion.enemies_mode2.remove(self)
        Configuratuion.enemies_mode3.add(self)
    ...
    def change_mode(self, from_mode, to_mode): # Дописать
        ...
        if to_mode == 1 and from_mode == 2:
            ...
            self.mode1_duration = Configuratuion.enemy_mode1_duration
            Configuratuion.enemies_mode2.remove(self)
            Configuratuion.enemies_mode1.add(self)
        if to_mode == 1 and from_mode == 3:
            ...
            self.mode1_start_time = pygame.time.get_ticks()
            self.mode1_duration = Configuratuion.enemy_mode1_duration
            Configuratuion.enemies_mode3.remove(self)
            Configuratuion.enemies_mode1.add(self)
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            ...
            Configuratuion.enemy_bullets.add(enemy_bullet)
    ...
    def update(self):
        if not self.frozen:
            ...
            # From 1 to 2
            if (self.mode == 1 and Configuratuion.player.hidden == False and
                now - self.mode1_start_time > self.mode1_duration):
                self.change_mode(1, 2)
            # From 2 to 3
            if (self.mode == 2 and Configuratuion.base.destroyed == False and
            ...
            if self.mode == 1: # Режим №1
                ...
                if (self.rect.right > Configuratuion.WIDTH or self.rect.left < 0 or
                    self.rect.bottom > Configuratuion.HEIGHT or self.rect.top < 0):
                    self.stop()
                    self.rotate()
                ...
                self.remove(Configuratuion.enemies_mode2)
                for enemy in Configuratuion.enemies_mode2:
                    ...
                    self.moving_blocked = False
                self.add(Configuratuion.enemies_mode2)
                ...
                if self.current_position != self.next_position:
                    self.remove(Configuratuion.enemies_mode3)
                    for enemy in Configuratuion.enemies_mode3:
                        ...
                        self.moving_blocked = False
                    self.add(Configuratuion.enemies_mode3)
                ...
                if self.current_position == self.next_position:
                    if self.rect.x < Configuratuion.base.rect.x:
                        self.direction = "right"
                    elif self.rect.y < Configuratuion.base.rect.y:
                        self.direction = "down"
                    elif self.rect.x > Configuratuion.base.rect.x:
                        self.direction = "left"
...
class FastEnemy(Enemy):
    def __init__(self, enemy_id, x):
        super().__init__(enemy_id, x)
        self.rand_image = random.choice(Configuratuion.enemy_images)[1]
        self.image = self.rand_image
        self.image.set_colorkey(Configuratuion.BLACK)
        self.rect = self.image.get_rect()
        self.rect.x = x
        self.rect.y = 0
        self.tank_type = "fast"
        self.speed = Configuratuion.enemy_speed * 1.4
        self.speedy = self.speed
...
class EnhancedEnemy(Enemy):
    def __init__(self, enemy_id, x):
        super().__init__(enemy_id, x)
        self.rand_image = random.choice(Configuratuion.enemy_images)[2]
        self.image = self.rand_image
        self.image.set_colorkey(Configuratuion.BLACK)
        self.rect = self.image.get_rect()
        self.rect.x = x
...
class HeavyEnemy(Enemy):
    def __init__(self, enemy_id, x):
        super().__init__(enemy_id, x)
        self.rand_image = random.choice(Configuratuion.enemy_images)[3]
        self.image = self.rand_image
        self.image.set_colorkey(Configuratuion.BLACK)
        self.rect = self.image.get_rect()
        self.rect.x = x
